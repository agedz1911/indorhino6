<div>
    <section class="breadcrumbs relative pb-0">
        ...
        <div class="py-16 lg:py-28 text-center relative">
            <h2 class="text-white uppercase text-2xl font-semibold tracking-wide lg:text-4xl">{{__('menu.faculty')}}
            </h2>
        </div>
    </section>
    <section class="pt-10 pb-24 px-5 lg:px-8 bg-base-100">
        <div class="w-full px-3 mb-5 lg:mb-10">
            <label class="input input-bordered flex items-center gap-2 w-full">
                <input wire:model.live.debounce.500ms="searchTerm" type="text"
                    class="border-none w-full focus:border-none" placeholder="Search Faculties" />
        ...
        <div class="tabs tabs-border justify-evenly">
            <input type="radio" name="my_tabs_2" class="tab" aria-label="International Faculties | Indonesian Faculties"
                checked="checked" />
            <div class="tab-content border-base-300 bg-base-100 p-10">
                <div class="flex flex-wrap justify-between items-start gap-4">
                    @foreach ($faculties as $faculty)
                    @if ($faculty->type_participants->where('name', 'Speaker')->isNotEmpty())


                    <button class="hover:shadow-lg hover:shadow-cyan-700/50 transition-all duration-300"
                        onclick="fac{{$loop->index}}.showModal()">
                        <div
                            class="card card-compact bg-[#D9F9F5]/20 ring-[#008795] ring-offset-base-100 ring-2 ring-offset-2 w-full md:max-w-xs">
                            <figure class="">
                            ...
                            <div class="card-body pt-0 flex flex-col justify-between">
                                ...
                                <span
                                    class="text-sm text-[#be124e] hover:cursor-pointer text-start">({{$faculty->country}})</span>
                                <p class="text-xs italic text-justify line-clamp-3">{{$faculty->cv}} </p>
                                <p class="text-end hover:underline hover:text-[#be124e] hover:cursor-pointer">
                                    {{__('home.read_more')}}...</p>
                            </div>
                        </div>
                    </button>
            ...
            <input type="radio" name="my_tabs_2" class="tab" aria-label="Pre-Event Webinar Faculties" />
            <div class="tab-content border-base-300 bg-base-100 p-10">
                <div class="flex flex-wrap justify-between items-start gap-4">
                    @foreach ($faculties as $faculty)
                    @if ($faculty->type_participants->where('name', 'Pre-event')->isNotEmpty())
